...
class Customer {
    ...
    public function getPowerConsumptionStats($month, $year) {
        ...
        /**
     * Get tenure / lease history for a customer (previous renewals, start/end, rent, service_charge)
     * Returns array of rows ordered by lease_start_date DESC
     */
    public function getTenureHistoryByCustomerId($customerId) {
        $this->db->query("SELECT id, lease_tenure, lease_start_date, lease_end_date, expected_rent, expected_service_charge, posting_time
                          FROM shops_history
                          WHERE shop_no = :customer_id
    ...
    /**
     * Summary of payments for a customer: total_paid_rent, total_paid_service_charge, last_payment_date
     * Uses customers_payments table (adjust table name if yours differs)
     */
    public function getPaymentSummaryByCustomerId($customerId) {
        $this->db->query("SELECT 
                            IFNULL(SUM(CASE WHEN payment_type = 'rent' THEN amount_paid ELSE 0 END), 0) AS total_rent_paid,
                            IFNULL(SUM(CASE WHEN payment_type = 'service_charge' THEN amount_paid ELSE 0 END), 0) AS total_sc_paid,
                            IFNULL(MAX(date_of_payment), '') AS last_payment_date
                          FROM account_general_transaction_new
                          WHERE shop_id = :customer_id");
        $this->db->bind(':customer_id', $customerId);
        $row = $this->db->single();
        return $row ?: array('total_rent_paid' => 0, 'total_sc_paid' => 0, 'last_payment_date' => '');
    }

    /**
     * Recent payments for a customer (limit optional)
     */
    public function getRecentPaymentsByCustomerId($customerId, $limit = 10) {
        $this->db->query("SELECT shop_id, shop_no, date_of_payment, payment_type, amount_paid, receipt_no, posting_officer_name
                          FROM account_general_transaction_new
                          WHERE shop_no = :customer_id
                          ORDER BY date_of_payment DESC
                          LIMIT :limit");
        $this->db->bind(':customer_id', $customerId);
        // PDO::PARAM_INT usage with DB wrapper that supports ->bind($param, $value, $type)
    ...
    public function getRecordUpdateAuthorization($shopId) {
        $this->db->query("SELECT * FROM record_update_authorization WHERE shop_id = :shop_id");
        $this->db->bind(':shop_id', $shopId);
        return $this->db->single();
    }

    public function getRecordVerificationAuthorization($shopId) {
        $this->db->query("SELECT * FROM record_verification_authorization WHERE shop_id = :shop_id");
        $this->db->bind(':shop_id', $shopId);
        return $this->db->single();
    }

    public function getShopsRenewalAuthorization($shopId) {
        $this->db->query("SELECT * FROM shops_renewal_authorization WHERE shop_id = :shop_id");
        $this->db->bind(':shop_id', $shopId);
        return $this->db->single();
    }

    public function getShopsVacancyAuthorization($shopId) {
        $this->db->query("SELECT * FROM shops_vacancy_authorization WHERE shop_id = :shop_id");
        $this->db->bind(':shop_id', $shopId);
        return $this->db->single();
    }

    // ========== New registration & renewal data ==========
    public function getCustomerNewRegistrationByShop($shopNo) {
        $this->db->query("SELECT * FROM customers_new_registration WHERE shop_no = :shop_no ORDER BY posting_time DESC");
        $this->db->bind(':shop_no', $shopNo);
        return $this->db->resultSet();
    }

    public function getRenewalDataUpdate($shopId) {
        $this->db->query("SELECT * FROM renewal_data_update WHERE shop_id = :shop_id ORDER BY posting_time DESC");
        $this->db->bind(':shop_id', $shopId);
        return $this->db->resultSet();
    }

    // ========== Reports & balance verification ==========
    public function getShopRecordReport($shopId) {
        $this->db->query("SELECT * FROM shop_record_report WHERE shop_id = :shop_id ORDER BY posting_time DESC");
        $this->db->bind(':shop_id', $shopId);
        return $this->db->resultSet();
    }

    public function getCustomerBalanceVerificationReport($shopNo) {
        $this->db->query("SELECT * FROM customer_balance_verification_report WHERE shop_no = :shop_no ORDER BY verification_time DESC");
        $this->db->bind(':shop_no', $shopNo);
        return $this->db->single();
    }

    // ========== Transactions & collection ==========
    public function getAccountTransactions($shopId, $category = 'Rent') {
        $this->db->query("SELECT * FROM account_general_transaction_new WHERE shop_id = :shop_id AND payment_category = :category AND approval_status = 'Approved' ORDER BY posting_time DESC");
        $this->db->bind(':shop_id', $shopId);
        $this->db->bind(':category', $category);
        return $this->db->resultSet();
    }

    public function getCollectionAnalysisByTransId($transId) {
        $this->db->query("SELECT * FROM collection_analysis WHERE trans_id = :trans_id");
        $this->db->bind(':trans_id', $transId);
        return $this->db->single();
    }

    public function getCollectionAnalysisArenaByTransId($transId) {
        $this->db->query("SELECT * FROM collection_analysis_arena WHERE trans_id = :trans_id");
        $this->db->bind(':trans_id', $transId);
        return $this->db->single();
    }

    // ========== Shops history & facility types ==========
    public function getShopsHistory($shopId) {
        $this->db->query("SELECT * FROM shops_history WHERE shop_no = :shop_no ORDER BY posting_time DESC");
        $this->db->bind(':shop_no', $shopId);
        return $this->db->resultSet();
    }

    public function getShopFacilityTypes() {
        $this->db->query("SELECT * FROM shops_facility_type ORDER BY facility_type ASC");
        return $this->db->resultSet();
    }
