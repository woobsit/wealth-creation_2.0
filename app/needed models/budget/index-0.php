...
$all_targets_yr = $target_manager->getAllOfficerTargets(null, 2025);
// print_r($all_targets_yr);
// exit;

// Get performance data
$performance_data = $budget_manager->getBudgetPerformanceRealTime($current_year, $selected_month);
...
//Array ( [acct_id] => 10400 [acct_desc] => Abattoir [budget_year] => 2025 [budgeted_amount] => 27748975.00 [actual_amount] => 12366900 [variance_amount] => -15382075 [variance_percentage] => -55.432948424221074 [performance_status] => Below Budget [performance_month] => [performance_year] => 2025 ) 
...
<body class="bg-gray-50 min-h-screen">
            ...
            <div class="flex justify-between h-16">
                <div class="flex items-center">
                    <div class="flex items-center mr-8">
                        ...
                        <a href="../index.php"><h1 class="text-xl font-bold text-gray-900"><?php echo APP_NAME; ?> - Budget Dashboard</h1></a>
                    </div>
                </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                ...
                <div class="text-right">
                    ...
                    <div class="mt-2 flex items-center justify-end">
                        <span class="text-sm <?php echo $achievement_percent >= 0 ? 'text-green-300' : 'text-red-300'; ?>">
                            <?php echo $achievement_percent >= 0 ? '+' : ''; ?><?php echo $achievement_percent; ?>% variance
                        </span>
                    </div>
                </div>
            </div>
        ...
        <!-- WC Budget Overview - Prominent Display -->
        <div class="bg-white rounded-xl shadow-lg border-2 border-blue-200 p-8 mb-8">
            <div class="text-center mb-6">
                <h2 class="text-3xl font-bold text-gray-900 mb-2">Wealth Creation Budget Overview</h2>
                <p class="text-gray-600 text-lg">As at <?php echo date('F Y'); ?> Performance Summary</p>
            </div>
            
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <!-- Total Budget -->
                <div class="text-center bg-blue-50 rounded-xl p-6 border border-blue-200">
                    <div class="mb-3">
                        <i class="fas fa-chart-pie text-blue-600 text-4xl"></i>
                    </div>
                    <div class="text-sm font-medium text-blue-600 uppercase tracking-wide mb-2">Total WC Budget</div>
                    <div class="text-4xl font-bold text-blue-900 mb-2">
                        ₦<?php echo number_format($total_budget); ?>
                    </div>
                    <div class="text-sm text-blue-700">Monthly Allocation</div>
                </div>
                
                <!-- Actual Collections -->
                <div class="text-center bg-green-50 rounded-xl p-6 border border-green-200">
                    <div class="mb-3">
                        <i class="fas fa-coins text-green-600 text-4xl"></i>
                    </div>
                    <div class="text-sm font-medium text-green-600 uppercase tracking-wide mb-2">Actual Collections</div>
                    <div class="text-4xl font-bold text-green-900 mb-2">
                        ₦<?php echo number_format($total_actual); ?>
                    </div>
                    <div class="text-sm text-green-700">Current Performance</div>
                </div>
                
                <!-- Variance -->
                <div class="text-center <?php echo $budget_variance >= 0 ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'; ?> rounded-xl p-6 border">
                    <div class="mb-3">
                        <i class="fas fa-chart-line <?php echo $budget_variance >= 0 ? 'text-green-600' : 'text-red-600'; ?> text-4xl"></i>
                    </div>
                    <div class="text-sm font-medium <?php echo $budget_variance >= 0 ? 'text-green-600' : 'text-red-600'; ?> uppercase tracking-wide mb-2">
                        Budget Variance
                    </div>
                    <div class="text-4xl font-bold <?php echo $budget_variance >= 0 ? 'text-green-900' : 'text-red-900'; ?> mb-2">
                        <?php echo $budget_variance >= 0 ? '+' : ''; ?><?php echo number_format($budget_variance, 1); ?>
                    </div>
                    <div class="text-sm <?php echo $budget_variance >= 0 ? 'text-green-700' : 'text-red-700'; ?>">
                        <?php echo $budget_variance >= 0 ? 'Above Target' : 'Below Target'; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-6 text-center">
                <div class="inline-flex items-center px-6 py-3 rounded-full text-lg font-semibold
                    <?php echo $budget_variance >= 10 ? 'bg-green-100 text-green-800' : 
                              ($budget_variance >= 0 ? 'bg-blue-100 text-blue-800' : 
                              ($budget_variance >= -10 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800')); ?>">
                    <i class="fas fa-<?php echo $budget_variance >= 10 ? 'trophy' : 
                                            ($budget_variance >= 0 ? 'thumbs-up' : 
                                            ($budget_variance >= -10 ? 'exclamation-triangle' : 'exclamation-circle')); ?> mr-2"></i>
                    <?php 
                    if ($budget_variance >= 10) echo 'Excellent Performance';
                    elseif ($budget_variance >= 0) echo 'Good Performance';
                    elseif ($budget_variance >= -10) echo 'Needs Attention';
                    else echo 'Critical Review Required';
                    ?>
                    <?php echo $achievement_percent >= 0 ? '+' : ''; ?><?php echo number_format($achievement_percent, 2); ?>%
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-6">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-lg font-semibold text-gray-900">
                    General Income lines Performance Summary as at <span class="px-2 rounded-full text-lg font-semibold bg-red-100 text-red-800"><?php echo date('F Y'); ?></span>
                </h3>
                ...
                <a href="mpr_income_lines_officers.php" 
                    ...
                    <div>
                        <div class="text-sm font-medium text-gray-900">Officer Performance Analysis</div>
            ...
            <div class="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold <?php echo $achievement_percent >= 0 ? 'text-green-600' : 'text-red-600'; ?>">
                            <?php echo $achievement_percent >= 0 ? '+' : ''; ?><?php echo number_format($achievement_percent, 2); ?>%
                        </p>
