        ...
        <div class="w-full lg:w-1/3 bg-white rounded-xl shadow p-4">
          ...
          <div class="flex flex-col space-y-3">
            ...
            foreach ($incomeLines as $line) {
                ...
                $svgPath = $line['icon'];
                echo "
                <button type='button' onclick=\"loadForm('$lineName', '$department')\" class='flex items-center bg-indigo-50 border border-indigo-200 rounded-lg px-4 py-2 hover:shadow-sm transition w-full text-left'>
...
function loadForm(line, department) {
    ...
    const remitId = '<?= isset($totalRemitted['remit_id']) ? $totalRemitted['remit_id'] : ''; ?>';
    const unposted = '<?= isset($totalRemitted['unposted']) ? $totalRemitted['unposted'] : 0; ?>';
    const remittanceDate = '<?= date('Y-m-d') ?>';
    const amtRemitted = '<?= isset($totalRemitted['remitted']) ? $totalRemitted['remitted'] : 0; ?>';

    const remittanceDropdown = sessionDept === 'Wealth Creation' ? `
    <div class="mb-3">
    ...
    if (line === 'Car Loading Ticket') {
        ...
        formArea.innerHTML = `
            ...
            <form method="post" action="post_payment_handler.php">
                ...
                <input type="hidden" name="posting_officer_id" value="<?= $userId; ?>">
                <input type="hidden" name="posting_officer_name" value="<?= $currentUser['full_name']; ?>">
                <input type="hidden" name="posting_officer_dept" value="${department}">
                ...
                <input type="text" name="date_of_payment" class="w-full p-2 border rounded bg-gray-100" value="<?= date('d/m/Y'); ?>" readonly />
                </div>
        ...
        formArea.innerHTML = `
            ...
            <form action="post_payment_handler.php" method="POST" class="space-y-4">
                ...
                <input type="text" name="customer_name" class="w-full p-2 border rounded" placeholder="Customer name">
                </div>
                ...
                <div>
                <label class="block font-medium">Date of Payment</label>
                <input type="date" name="date_of_payment" value="<?= date('Y-m-d'); ?>" class="w-full p-2 border rounded">
                </div>
