...
// --------------------- Custom Aggregated Officer Performance ---------------------

$start_of_year = date('Y-01-01');
$end_of_today = date('Y-m-d');

$selected_officers = isset($_GET['sstaff']) ? (is_array($_GET['sstaff']) ? $_GET['sstaff'] : [$_GET['sstaff']]) : [];

$income_line_summary = [];
$monthly_percentages = [];

if (!empty($selected_officers)) {
    foreach ($selected_officers as $officer_id) {
        foreach ($analyzer->getIncomeLines() as $line) {
            $total_collected = $analyzer->getTotalCollected($officer_id, $line['acct_id'], $start_of_year, $end_of_today);
            $total_target = $analyzer->getTotalTarget($officer_id, $line['acct_id'], $start_of_year, $end_of_today);

            if (!isset($income_line_summary[$line['name']])) {
                $income_line_summary[$line['name']] = ['collected' => 0, 'target' => 0];
            }
            $income_line_summary[$line['name']]['collected'] += $total_collected;
            $income_line_summary[$line['name']]['target'] += $total_target;

            // Monthly average percentage
            $monthly_data = $analyzer->getMonthlyCollectionData($officer_id, $line['id'], $start_of_year, $end_of_today);
            ...
            foreach ($monthly_data as $month) {
                ...
                if ($target > 0) {
                    $sum_percent += ($collected / $target) * 100;
                }
            }

            if (!isset($monthly_percentages[$line['name']])) {
                $monthly_percentages[$line['name']] = ['sum_percent' => 0, 'count' => 0];
            }

            $monthly_percentages[$line['name']]['sum_percent'] += $sum_percent;
            $monthly_percentages[$line['name']]['count'] += $month_count;
        }
    }

    // Prepare summary arrays
    $income_line_percentages = [];
    $monthly_avg_percentages = [];

    foreach ($income_line_summary as $line => $values) {
        $percent = ($values['target'] > 0) ? ($values['collected'] / $values['target']) * 100 : 0;
        $income_line_percentages[$line] = round($percent, 2);
    }

    foreach ($monthly_percentages as $line => $values) {
        $avg = ($values['count'] > 0) ? ($values['sum_percent'] / $values['count']) : 0;
        $monthly_avg_percentages[$line] = round($avg, 2);
    }
}
...
// Pagination for officer comparison
$total_officers = count($officer_comparison);
$total_pages = ceil($total_officers / $per_page);
$offset = ($page - 1) * $per_page;
$paginated_officers = array_slice($officer_comparison, $offset, $per_page);
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200">
                <div class="flex items-center justify-between">
                    <h3 class="text-lg font-semibold text-gray-900">Wealth Creation Officers Performance Comparison</h3>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <?php foreach ($paginated_officers as $index => $officer): ?>
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <?php $global_index = $offset + $index; ?>
                                    <?php if ($global_index < 3): ?>
                                        <div class="w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-bold
                                            <?php echo $global_index === 0 ? 'bg-yellow-500' : ($global_index === 1 ? 'bg-gray-500' : 'bg-orange-500'); ?>">
                                            <?php echo $global_index + 1; ?>
                                        </div>
                                    <?php else: ?>
