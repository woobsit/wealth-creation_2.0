...
class GeneralPerformanceAnalyzer {
    ...
    public function getOfficerOverallPerformance($year = null) {
        ...
        $this->db->query("
            ...
            GROUP BY s.user_id, s.full_name, s.department, s.phone_no
    ...
    public function getOfficerMonthlyBreakdown($officer_id, $year = null) {
        ...
        for ($month = 1; $month <= 12; $month++) {
            ...
            $monthly_data[] = [
                ...
                'transactions' => isset($result['transactions']) ? $result['transactions'] : 0,
                'total_amount' => isset($result['total_amount']) ? $result['total_amount'] : 0,
                'working_days' => isset($result['working_days']) ? $result['working_days'] : 0,
                'income_lines_used' => isset($result['income_lines_used']) ? $result['income_lines_used'] : 0,
                'daily_average' => (isset($result['working_days']) && $result['working_days'] > 0) ? (isset($result['total_amount']) ? $result['total_amount'] : 0) / $result['working_days'] 
                : 0
            ];
        }
    ...
    public function getOfficerRanking($year = null) {
        ...
        $percentiles = [
            'p90' => isset($collections[floor($count * 0.9)]) ? $collections[floor($count * 0.9)] : 0,
            'p75' => isset($collections[floor($count * 0.75)]) ? $collections[floor($count * 0.75)] : 0,
            'p50' => isset($collections[floor($count * 0.5)]) ? $collections[floor($count * 0.5)] : 0,
            'p25' => isset($collections[floor($count * 0.25)]) ? $collections[floor($count * 0.25)] : 0
        ];
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <ul class="text-sm text-gray-600 space-y-1">
                        ...
                        usort($consistent, function($a, $b) {
                            if ($b['activity_rate'] == $a['activity_rate']) {
                                return 0;
                            }
                            return ($b['activity_rate'] < $a['activity_rate']) ? -1 : 1;
                        });
                        ?>
