...
// $date_from = isset($_GET['d1']) ? $_GET['d1'] : null;
// $date_to = isset($_GET['d2']) ? $_GET['d2'] : null;
// $ledgerTable = null;

// if ($acct_id) {
//     // Fetch ledger table name
//     $db->query("SELECT acct_table_name FROM accounts WHERE acct_id = :acct_id");
//     $db->bind(":acct_id", $acct_id);
//     $acct_result = $db->single();

//     if ($acct_result && isset($acct_result['acct_table_name'])) {
//         $ledgerTable = $acct_result['acct_table_name'];
//     } else {
//         die("Invalid account selected.");
//     }

//     // Decode and safely convert dates to Y-m-d
//     if ($date_from && $date_to) {
//         $date_from = urldecode($date_from);
//         $date_to = urldecode($date_to);

//         // Try dd/mm/yyyy
//         if (preg_match("#^(\d{2})/(\d{2})/(\d{4})$#", $date_from, $from_parts) &&
//             preg_match("#^(\d{2})/(\d{2})/(\d{4})$#", $date_to, $to_parts)) {

//             $date_from = "{$from_parts[3]}-{$from_parts[2]}-{$from_parts[1]}";
//             $date_to = "{$to_parts[3]}-{$to_parts[2]}-{$to_parts[1]}";
...
//         } else {
//             die("Invalid date format. Use dd/mm/yyyy or yyyy-mm-dd.");
//         }
//     }
// }
