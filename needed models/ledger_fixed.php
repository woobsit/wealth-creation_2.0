...
// Accept GET parameters: acct_id, d1, d2

function convertDate($dateStr) {
    $date = DateTime::createFromFormat('d/m/Y', $dateStr);
    if ($date) return $date->format('Y-m-d');

    // fallback if already in Y-m-d or another valid format
    $date = DateTime::createFromFormat('Y-m-d', $dateStr);
    return $date ? $date->format('Y-m-d') : null;
}

$acct_id = isset($_GET['acct_id']) ? intval($_GET['acct_id']) : 0;
$d1_raw = isset($_GET['d1']) ? $_GET['d1'] : '';
$d2_raw = isset($_GET['d2']) ? $_GET['d2'] : '';

$start_date = convertDate($d1_raw);
$end_date   = convertDate($d2_raw);

if (!$start_date || !$end_date || !$acct_id) {
    die("Invalid input. Please check account ID and date format.");
}
// Now use $acct_id, $start_date, $end_date in your queries
