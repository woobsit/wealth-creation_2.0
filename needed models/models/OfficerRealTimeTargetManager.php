...
class OfficerRealTimeTargetManager { 
    private $db;
    ...
    public function saveOfficerTarget($data) {
        ...
        try {
            // Normalize numbers
            $monthly_target = (float)$data['monthly_target'];

            // Calculate daily target (excluding Sundays)
            $working_days = $this->getWorkingDaysInMonth((int)$data['target_month'], (int)$data['target_year']);
            $daily_target = $working_days > 0 ? ($monthly_target / $working_days) : 0;

            // ---- EDIT / UPDATE PATH ----
            if (isset($data['target_id']) && $data['target_id']) {
                // Bind ONLY placeholders that exist in this UPDATE statement
                $this->db->query("
                    UPDATE officer_monthly_targets
                    SET
                ...
                $this->db->bind(':monthly_target', $monthly_target);
                ...
                return array('success' => true, 'message' => 'Officer target updated successfully!');
            }

            // ---- CREATE / UPSERT PATH ----
            // We only fetch officer/account when inserting (needed for officer_name, department, acct_desc)
            ...
            // Insert (no updated_by here). If row exists, update selected fields (still no updated_by).
            $this->db->query("
                INSERT INTO officer_monthly_targets (
                    officer_id, officer_name, department,
                    target_month, target_year,
                    acct_id, acct_desc,
                    monthly_target, daily_target, status, created_by
                ) VALUES (
                    :officer_id, :officer_name, :department,
                    :target_month, :target_year,
                    :acct_id, :acct_desc,
            ...
            // Bind ONLY placeholders present in the INSERT/UPSERT statement above
            $this->db->bind(':officer_id', $data['officer_id']);
            $this->db->bind(':officer_name', isset($officer['full_name']) ? $officer['full_name'] : null);
            $this->db->bind(':department', isset($officer['department']) ? $officer['department'] : null);
            $this->db->bind(':target_month', (int)$data['target_month']);
            $this->db->bind(':target_year', (int)$data['target_year']);
            $this->db->bind(':acct_id', $data['acct_id']);
            $this->db->bind(':acct_desc', isset($account['acct_desc']) ? $account['acct_desc'] : null);
            $this->db->bind(':monthly_target', $monthly_target);
            $this->db->bind(':daily_target', $daily_target);
            $this->db->bind(':status', $data['status']);
            ...
            return array('success' => true, 'message' => 'Officer target saved successfully!');

        } catch (Exception $e) {
            $this->db->cancelTransaction();
            return array('success' => false, 'message' => 'Error saving target: ' . $e->getMessage());
        }
    }
    ...
    /**
     * Get officer performance tracking
     */
    public function getnupdateOfficerPerformance($month, $year) {
        $this->db->beginTransaction();
        ...
        try {
            ...
            $report = []; // store officer analysis for return
            ...
            foreach ($targets as $target) {
                ...
                // Officer rating based on achievement
                if ($percentage >= 100) {
                    $rating = "Excellent";
    ...
    public function getOfficerPerformanceSummary($officer_id, $month, $year) {
        $this->db->query("
            SELECT 
                ...
                AVG(COALESCE(opt_track.performance_score, 0)) as avg_score,
                COUNT(CASE WHEN opt_track.performance_grade IN ('A+', 'A') THEN 1 END) as excellent_count,
                COUNT(CASE WHEN opt_track.performance_grade IN ('B+', 'B') THEN 1 END) as good_count,
                COUNT(CASE WHEN opt_track.performance_grade IN ('C+', 'C', 'D', 'F') THEN 1 END) as poor_count
