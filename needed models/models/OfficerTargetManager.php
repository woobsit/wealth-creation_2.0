...
class OfficerTargetManager {
    ...
    public function saveOfficerTarget($data) {
        ...
        try {
            ...
            $daily_target = $working_days > 0 ? $data['monthly_target'] / $working_days : $data['daily_target'];
    ...
    /**
     * Get working days in month (excluding Sundays only)
     */
    private function getWorkingDaysInMonth($month, $year) {
        $month = (int)$month;
        $year  = (int)$year;
     ...
     * - Optional: exclude Saturdays
     * - Optional: exclude specific holidays (array of 'Y-m-d' strings)
     */
    // private function getWorkingDaysInMonthx($month, $year, array $holidays = [], bool $excludeSaturday = false) {
    ...
    //         if ($excludeSaturday && $w === 6) continue; // optionally exclude Saturdays
    //         if (!empty($holidays) && in_array($dt->format('Y-m-d'), $holidays, true)) continue;
