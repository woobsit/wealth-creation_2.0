...
<head>
    ...
    <title>Audit Approval - Transaction Posting</title>
...
<body class="bg-gray-50">
    <div class="container mx-auto px-4 py-6">
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex justify-between items-center mb-6">
                <div>
                    <h1 class="text-3xl font-bold text-gray-800">Audit Department - Final Approval</h1>
                    <p class="text-gray-600">Level 3: Compliance check and posting to ledger</p>
                </div>
                <div class="bg-green-100 border border-green-300 rounded-lg px-4 py-2">
                    <p class="text-sm text-green-800"><strong><?= count($transactions) ?></strong> FC-Approved</p>
                </div>
            </div>

            <?php if (empty($transactions)): ?>
            <div class="bg-blue-50 border border-blue-200 rounded p-6 text-center">
                <p class="text-blue-800 font-medium">✓ No FC-approved transactions pending audit review</p>
            </div>
            <?php else: ?>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm">
                                <div class="flex gap-2 justify-center">
                                    <form method="POST" action="?action=audit-approve" style="display:inline;">
                                        <input type="hidden" name="remit_id" value="<?= $txn['remit_id'] ?>">
                                        <input type="hidden" name="action" value="approve">
                                        <button type="submit" class="px-3 py-1 bg-green-600 text-white text-xs rounded hover:bg-green-700 font-medium">
                                            ✓ Post to Ledger
                                        </button>
                                    </form>
            ...
            <div class="mt-6 bg-green-50 border-l-4 border-green-500 rounded p-4">
                <p class="text-sm text-green-800">
                    <strong>Role:</strong> As Audit Department, conduct final compliance verification and post fully-approved transactions 
                    to the General Ledger. All transactions in the ledger are final and comply with all organizational policies.
                </p>
            </div>
        </div>
    </div>
    ...
    <div id="rejectModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg shadow-lg p-6 max-w-md">
            ...
            <form method="POST" action="?action=audit-approve">
